...
| `fonts.local.read` | מניית הגופנים המותקנים מתוך הדף (`queryLocalFonts()`), עם השם המלא של כל face וקובץ הגופן שלו. **הרשאת דפדפן, לא RPC** — ראו „מניית הגופנים המותקנים”. ברירת מחדל: **מאושרת**. מ-0.9.98 |
...
דף התוסף נטען מ-`file://`, ושם `navigator.clipboard.read()` ו-`readText()` **נדחים** — לא בגלל הקשר לא-מאובטח (`isSecureContext` הוא `true`) אלא בגלל הרשאת ה-`clipboard-read` של הדפדפן. אוצריא מעניקה אותה רק לתוסף שהצהיר על `clipboard.read` במניפסט **ושהמשתמש אישר** לו אותה; ברירת המחדל כבויה, והמשתמש יכול לכבות בכל עת. כל בקשת הרשאה אחרת של הדף — מצלמה, מיקרופון, מיקום — נדחית, ואין הרשאת מניפסט שפותחת אותה. היחידה שכן, מלבד זו, היא מניית הגופנים — ראו „מניית הגופנים המותקנים”.
...
### מניית הגופנים המותקנים

`queryLocalFonts()` — ה-Local Font Access API — מחזיר את הגופנים המותקנים במחשב, ולכל אחד `family`, `fullName`, `style`, ו-`blob()` עם קובץ הגופן עצמו. הרשאת ה-`local-fonts` שהוא דורש מוענקת לתוסף שהצהיר על `fonts.local.read` במניפסט; ברירת המחדל **מאושרת**, והמשתמש יכול לכבות בכל עת.

```jsonc
{ "permissions": ["fonts.local.read"], "minAppVersion": "0.9.98" }
```

**מתי זה נדרש ומתי לא.** ל-`fonts.listInstalled` יש תפקיד משלה והיא לא מוחלפת: היא זולה, היא מסווגת כיסוי תווים (`scripts: ['hebrew']`) והיא עונה בכל פלטפורמה. אבל היא מונה דרך GDI, כלומר **משפחות של ארבעה סגנונות** — וּווריאנט משקל כמו `HadasaNew Light` או `Segoe UI Semilight` מקופל לתוך משפחת הבסיס או נושא שם שה-WebView אינו פותר. תוסף שמציג בורר גופנים בסגנון וורד צריך את השם המלא של כל face, וזה מה שה-API הזה נותן.

ושתי נקודות שחוסכות אבחון:

* **`blob()` הוא מה שהופך את הרשימה לשמישה.** שם שהמנייה מחזירה אינו בהכרח שם ש-`font-family` פותר — GDI ו-DirectWrite אינם מסכימים על כל וריאנט משקל. הבייטים מאפשרים לדף להזריק `@font-face` בשם שהוא בחר, ולצייר את הגופן במקום ליפול ל-fallback שנראה כמו גופן אחר.
* **אין `fonts.queryLocal` ב-`Otzaria.call`.** כמו קריאת הלוח, זו הרשאת דפדפן ולא מסלול RPC: היא נאכפת ב-`onPermissionRequest`, ומה שהתוסף קורא לו הוא ה-API הרגיל של הדפדפן. גם כאן דחייה נרשמת בלוג הריצה.
