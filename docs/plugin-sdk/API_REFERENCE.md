...
(ברירת מחדל `GET`), `headers` (אובייקט, אופציונלי), `body` (מחרוזת,
אופציונלי) ו-`timeoutMs` (מספר שלם חיובי). חסם הזמן חל על הבקשה
כולה, כולל קריאת הגוף; ברירת המחדל היא 30,000 והמקסימום 120,000
מילישניות.
